...
class HtmlContentUtils(object):
    ...
    def setItemCounts(self, itemNameD, deliveryType='archive'):
        ...
        logger.debug("+HtmlContentUtils.setItemCounts() items      in archive count %d\n" % len(self.__itemCounts[deliveryType]))
        logger.debug("+HtmlContentUtils.setItemCounts() categories in archive count %d\n" % len(self.__categoryCounts[deliveryType]))
    ...
    def __renderTable(self, rowList, columnNameList, newLines='verbatim', columnNameFormat='html', dataFormat='ascii', markupMath=False):
        ...
        html.clear()
        html.beginTable(columnNameList=columnNameList, format=columnNameFormat)
        for row in rowList:
            html.addTableRow(rowValueList=row, format=dataFormat, newLines=newLines, markupMath=markupMath)
    ...
    def makeDictionaryIndex(self, downloadPath, dictionaryName, title=None, description=None, authors=None, maintainers=None, order='reverse'):
        ...
        #
        historyHtmlList = self.__renderTable(rowList=historyList, columnNameList=['Version', 'Revision&nbsp;Date', 'Revision&nbsp;Description'],
                                             columnNameFormat='html', dataFormat='ascii')
        #
        self.__html.clear()

        self.__html.beginContainer(type='row')
        iconText, addClass = self.__getIconAnchorAndClass(contentIconType='general', cssClass='pull-right')
        self.__html.beginPanel('General ' + iconText, style='panel-default ' + addClass, format='html')
        ...
        self.__html.beginPanel('Downloads ' + iconText, style='panel-default ' + addClass, format='html')
        self.__html.beginContainer(type="col-md-10 col-md-offset-1")
        dList = []
        downloadClass = "btn-lg btn-wwpdb-green"
        url = os.path.join(downloadPath, dictionaryName + '.dic')
        dList.append(self.__mU.getButtonAnchor(contentLabel="Dictionary Text", url=url, format="ascii", cssClassAdd=downloadClass))

        url = os.path.join(downloadPath, dictionaryName + '.dic.gz')
        dList.append(self.__mU.getButtonAnchor(contentLabel="Dictionary Text (gz)", url=url, format="ascii", cssClassAdd=downloadClass))
        #
        ...
        self.__html.addAccordionPanel(title='Dictionary Revision History', panelTextList=historyHtmlList, panelId='p1', panelGroupId='pg1',
                                      openFlag=True, toggleText='View/Hide revision history list')
    ...
    def makeSupportingDataIndex(self):
        ...
        htmlList = self.__renderTable(rowList=rowList, columnNameList=['Version', 'Revision&nbsp;Date', 'Revision&nbsp;Description'],
                                      columnNameFormat='html', dataFormat='ascii')
        self.__html.addAccordionPanel(title='Dictionary Revision History', panelTextList=htmlList, panelId='sdp1', panelGroupId='pg1',
                                      openFlag=False, toggleText='View/Hide revision history list')

        #
        rowList = self.__dApi.getDataTypeList()
        htmlList = self.__renderTable(rowList=rowList, columnNameList=['Data&nbsp;Type&nbsp;Code', 'Primitive&nbsp;Type&nbsp;Code', 'Regular&nbsp;Expression', 'Description'],
                                      columnNameFormat='html', dataFormat='ascii')
        self.__html.addAccordionPanel(title='Data Type List', panelTextList=htmlList, panelId='sdp2', panelGroupId='pg1',
                                      openFlag=False, toggleText='View/Hide data type list')

        rowList = self.__dApi.getSubCategoryList()
        htmlList = self.__renderTable(rowList=rowList, columnNameList=['SubCategory&nbsp;Identifier', 'Description'],
                                      columnNameFormat='html', dataFormat='ascii')
        self.__html.addAccordionPanel(title='SubCategory List', panelTextList=htmlList, panelId='sdp3', panelGroupId='pg1',
                                      openFlag=False, toggleText='View/Hide subcategory list')

        rowList = self.__dApi.getUnitsList()
        htmlList = self.__renderTable(rowList=rowList, columnNameList=['Units&nbsp;Identifier', 'Description'],
                                      columnNameFormat='html', dataFormat='ascii', markupMath=True)
        self.__html.addAccordionPanel(title='Units List', panelTextList=htmlList, panelId='sdp4', panelGroupId='pg1',
                                      openFlag=False, toggleText='View/Hide units list')

        rowList = self.__dApi.getUnitsConversionList()
        htmlList = self.__renderTable(rowList=rowList, columnNameList=['From&nbsp;Units&Identifier', 'To&nbsp;Units&Identifier', 'Operator', 'Conversion&nbsp;Factor'],
                                      columnNameFormat='html', dataFormat='ascii')
        self.__html.addAccordionPanel(title='Units Conversion List', panelTextList=htmlList, panelId='sdp5', panelGroupId='pg1',
                                      openFlag=False, toggleText='View/Hide units conversion list')
    ...
    def __renderLinkGroup(self, contentNameList, contentType):
        ...
        for contentName in contentNameList:
            ...
            html.addLinkListItem(linkUrl, contentName, format='ascii', active=False)
    ...
    def __renderLinkGroupWithIcons(self, contentNameList, contentIconTypeList=None, contentType='Items'):
        ...
        if contentIconTypeList is None:
            contentTypeList = ['none' for c in contentNameList]
        html = HtmlComponentMarkupUtils(verbose=self.__verbose)
        html.clear()
        html.beginListGroup()
        for contentName, contentIconType in zip(contentNameList, contentIconTypeList):
            linkUrl = self.__pI.getContentTypeObjUrl(contentObjName=contentName, contentType=contentType)
            linkAnchor = self.__mU.getAnchor(contentUrl=linkUrl, contentLabel=contentName, cssClassString=None, format="ascii")
            ...
            html.addListItem(listValue, format='html', active=False, cssClass=addClass)
        html.endListGroup()
        return html.getHtmlList()
    ...
    def makeCategoryGroupPage(self, groupName):
        ...
        if len(catNameList) > 0:
            ...
            self.__html.addAccordionPanel(title=groupName, subTitle=descriptionS, panelTextList=catHtmlList, panelId=pIdS, panelGroupId='pg1',
                                          openFlag=openF, toggleText='View/Hide category list', topId=groupName)
        self.__html.endContainer()
        return self.__html.getHtmlList()
    ...
    def makeCategoryGroupIndex(self, openFirst=True, leadingList=None):
        ...
        if leadingList is not None:
            groupNameList = []
            for g in leadingList:
                groupNameList.append(g)
            for g in gList:
                if g in leadingList:
                    continue
                groupNameList.append(g)
    ...
    def __getOrderedItemNameList(self, categoryName):
        ...
        for keyItemName in keyItemNameList:
            if keyItemName in tList:
                ...
                logger.debug("Category %s is missing key definition for %s\n" % (categoryName, keyItemName))
        itemNameList = []
    ...
    def makeItemCategoryIndex(self, openFirst=True):
        ...
        for ii, catName in enumerate(catNameList):
            ...
            categoryTitle = self.__mU.getAnchor(contentUrl=categoryUrl, contentLabel=catName, cssClassString="my-link-color", format="ascii")
            if openFirst and ii == 0:
                openF = True
            else:
                openF = False
            ...
            self.__html.addAccordionPanel(title=categoryTitle, subTitle=subTitle,
                                          panelTextList=htmlList, panelId=pIdS, panelGroupId='pg1',
                                          openFlag=openF, toggleText='Item list view/hide')
    ...
    def __makeItemCategoryIndex(self, categoryNameList, pgId='cita', openFirst=False):
        ...
        idPrefix = pgId
        html = HtmlComponentMarkupUtils(verbose=self.__verbose)
        html.beginContainer(type="col-md-10 col-md-offset-1")
        #
        ...
        for ii, categoryName in enumerate(categoryNameList):
            ...
            categoryTitle = self.__mU.getAnchor(contentUrl=categoryUrl, contentLabel=categoryName, cssClassString="my-link-color", format="ascii")
            if openFirst and ii == 0:
                openF = True
            else:
                openF = False
            ...
            html.addAccordionPanel(title=categoryTitle, subTitle=subTitle,
                                   panelTextList=htmlList, panelId=pIdS, panelGroupId=pgId,
                                   openFlag=openF, toggleText='Item list view/hide')
    ...
    def makeItemCategoryAlphaIndex(self, openFirst=False):
        ...
        for ii, c in enumerate(cKeys):
            abbrevNameList = cIdx[c]
            pIdS = idPrefix + str(ii + 1)
            pgIdS = idPrefix + c + str(ii + 1)
            ...
            title = self.__mU.getIndexTitle(c.upper())
            self.__html.addAccordionPanel(title=title, subTitle=subTitle,
                                          panelTextList=htmlList, panelId=pIdS, panelGroupId=pgId,
                                          openFlag=openF, toggleText='Category list view/hide')
    ...
    def makeCategoryIndex(self):
        ...
        for catName in catNameList:
            ...
            self.__html.addLinkListItem(linkUrl, catName, format='ascii', active=False)
    ...
    def __contentPanelList(self, panelTitle, idPrefix, contentList):
        ...
        for ii, eTup in enumerate(contentList):
            ...
            ht = self.__mU.getFormatted(eTup[0], wrapper='pre', format='none')
            if ii == 0:
                opF = True
            else:
                opF = False
            html.addAccordionPanel(title=subPanelTitle + str(ii + 1),
                                   subTitle=None,
                                   panelTextList=[ht],
                                   panelId=pId,
                                   panelGroupId=pgIdI,
                                   openFlag=opF,
                                   toggleText='View/Hide ' + subPanelTitle)
        html.endPanel()
        return html.getHtmlList()
    ...
    def __contentPanelListAlt(self, panelTitle, subPanelTitle, idPrefix, contentList):
        ...
        for ii, eTup in enumerate(contentList):
            ...
            if ii == 0:
                ...
                opF = False
            html.addAccordionPanel(title=subPanelTitle + str(ii + 1), subTitle=None, panelTextList=[ht],
                                   panelId=pId, panelGroupId=pgIdI, openFlag=opF, toggleText='View/Hide ' + subPanelTitle)
        ...
        html.addAccordionPanel(title=panelTitle, subTitle=subTitle, panelTextList=hL,
                               panelId=pId, panelGroupId=pgIdI, openFlag=True, toggleText='View/hide ' + panelTitle)
        html.endAccordionPanelGroup()
        return html.getHtmlList()
    ...
    def __renderExamples(self, exampleList, titleSingle, titleMulti, idSuffix='a'):
        ...
        if exCount > 0:
            if exCount > 1:
                ...
                eTup = exampleList[0]
                ht = self.__mU.getFormatted(eTup[0], wrapper='pre', format='none')
                html.addAccordionPanel(title=titleSingle, subTitle=None, panelTextList=[ht], panelId=pId,
                                       panelGroupId=pgId, openFlag=True, toggleText='View/hide Example')
        return html.getHtmlList()
    ...
    def __addCategoryFigures(self, categoryName):
        ...
        self.__html.beginContainer(type='row')
        self.__html.beginPanel('Category Relationship Diagrams ' + iconText, style='panel-default ' + addClass, format='html')
        self.__html.beginContainer(type='row')

        if okFull:
            if imgCount > 2:
                self.__html.beginContainer(type='col-md-1')
            else:
                self.__html.beginContainer(type='col-md-1 col-md-offset-1')
            ...
            bText = self.__mU.getButtonAnchor(contentLabel=htText, url="#image-modal-full-1", format="html",
                                              cssClassAdd="btn-wwpdb-green btn-wwpdb-lg", dataAttributes='data-toggle="modal"')
            self.__html.addContent([bText])
            self.__html.endContainer()
            #
            self.__html.beginContainer(type='col-md-3')
        ...
        if okAbbrev:
            self.__html.beginContainer(type='col-md-1')
            htText = self.__mU.getImage(src="/assets/images/cr-figure-icon.svg", altText="Category relationship diagram", width=70, height=70, cssClassAdd="img-thumbnail")
            bText = self.__mU.getButtonAnchor(contentLabel=htText, url="#image-modal-abbrev-1", format="html",
                                              cssClassAdd="btn-wwpdb-green btn-wwpdb-lg", dataAttributes='data-toggle="modal"')
            self.__html.addContent([bText])
            self.__html.endContainer()
            #
        ...
        if okCc:
            self.__html.beginContainer(type='col-md-1')
            htText = self.__mU.getImage(src="/assets/images/cr-figure-icon.svg", altText="Category relationship diagram", width=70, height=70, cssClassAdd="img-thumbnail")
            bText = self.__mU.getButtonAnchor(contentLabel=htText, url="#image-modal-cc-1", format="html",
                                              cssClassAdd="btn-wwpdb-green btn-wwpdb-lg", dataAttributes='data-toggle="modal"')
            self.__html.addContent([bText])
            self.__html.endContainer()
            #
        ...
        if okPrd:
            self.__html.beginContainer(type='col-md-1')
            htText = self.__mU.getImage(src="/assets/images/cr-figure-icon.svg", altText="Category relationship diagram", width=70, height=70, cssClassAdd="img-thumbnail")
            bText = self.__mU.getButtonAnchor(contentLabel=htText, url="#image-modal-bird-1", format="html",
                                              cssClassAdd="btn-wwpdb-green btn-wwpdb-lg", dataAttributes='data-toggle="modal"')
            self.__html.addContent([bText])
            self.__html.endContainer()
            #
        ...
        if okFamily:
            self.__html.beginContainer(type='col-md-1')
            htText = self.__mU.getImage(src="/assets/images/cr-figure-icon.svg", altText="Category relationship diagram", width=70, height=70, cssClassAdd="img-thumbnail")
            bText = self.__mU.getButtonAnchor(contentLabel=htText, url="#image-modal-bird-family-1", format="html",
                                              cssClassAdd="btn-wwpdb-green btn-wwpdb-lg", dataAttributes='data-toggle="modal"')
            self.__html.addContent([bText])
            self.__html.endContainer()
            #
        ...
        if okFull:
            ...
            self.__html.addModalImageDialog(modalId='image-modal-full-1', modalTitle=titleText, imagePath=imgFileUrl,
                                            imageText=titleText, cssModalSect='my-image-scrollable', cssModalClose='btn-default')
        ...
        if okAbbrev:
            ...
            self.__html.addModalImageDialog(modalId='image-modal-abbrev-1', modalTitle=titleText, imagePath=imgFileUrl,
                                            imageText=titleText, cssModalSect='my-image-scrollable', cssModalClose='btn-default')

        if okCc:
            imgFileUrl = os.path.join(imgDirUrl, categoryName + "_neighbors_cc.svg")
            titleText = "Category Relationship Diagram for %s" % str(categoryName).upper()
            self.__html.addModalImageDialog(modalId='image-modal-cc-1', modalTitle=titleText, imagePath=imgFileUrl,
                                            imageText=titleText, cssModalSect='my-image-scrollable', cssModalClose='btn-default')

        if okPrd:
            imgFileUrl = os.path.join(imgDirUrl, categoryName + "_neighbors_prd.svg")
            titleText = "Category Relationship Diagram for %s" % str(categoryName).upper()
            self.__html.addModalImageDialog(modalId='image-modal-bird-1', modalTitle=titleText, imagePath=imgFileUrl,
                                            imageText=titleText, cssModalSect='my-image-scrollable', cssModalClose='btn-default')

        if okFamily:
            imgFileUrl = os.path.join(imgDirUrl, categoryName + "_neighbors_family.svg")
            titleText = "Category Relationship Diagram for %s" % str(categoryName).upper()
            self.__html.addModalImageDialog(modalId='image-modal-bird-family-1', modalTitle=titleText, imagePath=imgFileUrl,
                                            imageText=titleText, cssModalSect='my-image-scrollable', cssModalClose='btn-default')
    ...
    def __renderCategoryPage(self, categoryName):
        ...
        if ((mandatoryCode is None) or (len(mandatoryCode) < 1) or (mandatoryCode in ['.', '?'])):
            mandatoryCode = 'No'
            if self.__debug:
                logger.debug("+HtmlContentUtils.__renderCategoryPage() no mandatory code for %s\n" % categoryName)
        #
        groupAnchorList = []
        ...
        for groupName in categoryGroupList:
            if groupName not in ['inclusive_group']:
                ...
                groupAnchor = self.__mU.getAnchor(contentUrl=groupUrl, contentLabel=groupName, cssClassString="my-link-color", format="ascii")
        ...
        catAnchor = self.__mU.getAnchor(contentUrl=catUrl, contentLabel=categoryName, cssClassString="my-link-color", format="ascii")
        ...
        self.__html.beginContainer(type='row')
        self.__html.beginPanel('Category Description ' + iconText, style='panel-default ' + addClass, format='html')
        ht = self.__mU.getFormatted(description, wrapper='pre', format='ascii')
        #
        ...
        if self.__notNull(altDescription):
            ...
            self.__html.beginContainer(type='row')
            self.__html.beginPanel('Additional Descriptive Information for  Depositors ' + iconText, style='panel-default ' + addClass, format='html')
            ht = self.__mU.getFormatted(description, wrapper='pre', format='none')
            self.__html.addContent([ht])
            self.__html.endPanel()
            self.__html.endContainer()
        ...
        htmlList = self.__renderExamples(altExampleList, 'Additional Category Example for Depositors',
                                         'Additional Category Examples for Depositors', idSuffix='b')
        self.__html.beginContainer(type='row')
        self.__html.addContent(htmlList)
        self.__html.endContainer()
        #
        ...
        self.__html.beginContainer(type='row')
        self.__html.beginPanel('Key Data Items ' + iconText, style='panel-default ' + addClass, format='html')

        self.__html.beginInLineList()
        for keyItemName in keyItemNameList:
            itemUrl = self.__pI.getContentTypeObjUrl(contentObjName=keyItemName, contentType='Items')
            itemAnchor = self.__mU.getAnchor(contentUrl=itemUrl, contentLabel=keyItemName, cssClassString="my-link-color", format="ascii")
            listValue = itemAnchor
            self.__html.addInLineListItem(listValue, format='html')
        ...
        if (False):
            categoryGroupList = sorted(self.__dApi.getCategoryGroupList(categoryName))
            self.__html.beginContainer(type='row')
            self.__html.beginPanel('Category Groups', style='panel-default', format='html')
            self.__html.beginInLineList()
            for groupName in categoryGroupList:
                if groupName not in ['inclusive_group']:
                    groupUrl = self.__pI.getContentTypeObjUrl(contentObjName=groupName, contentType='Groups')
                    groupAnchor = self.__mU.getAnchor(contentUrl=groupUrl, contentLabel=groupName, cssClassString="my-link-color", format="ascii")
                    self.__html.addInLineListItem(groupAnchor, format='html')
            self.__html.endInLineList()
            self.__html.endPanel()
            self.__html.endContainer()
        ...
        itemCount, htmlList = self.__renderItemLinkList(categoryName)
        self.__html.beginContainer(type='row')
        self.__html.addAccordionPanel(title='Category Data Items', subTitle=None, panelTextList=htmlList, panelId='pit0',
                                      panelGroupId='pgit0', openFlag=True, toggleText='View/hide item list')
        self.__html.endContainer()
    ...
    def __addPanelWithInLineItemList(self, itemNameList, panelTitle, contentObjName='default'):
        ...
        self.__html.beginContainer()
        self.__html.beginPanel(panelTitle + iconText, style='panel-default ' + addClass, format='html')
        self.__html.beginInLineList()
        for itemName in itemNameList:
            itemUrl = self.__pI.getContentTypeObjUrl(contentObjName=itemName, contentType='Items')
            itemAnchor = self.__mU.getAnchor(contentUrl=itemUrl, contentLabel=itemName, cssClassString="my-link-color", format="ascii")
            listValue = itemAnchor
            self.__html.addInLineListItem(listValue, format='html')
        self.__html.endInLineList()
        self.__html.endPanel()
    ...
    def __addPanelWithInLineList(self, valueList, panelTitle, contentObjName='default', format='ascii'):
        """
        """
        iconText, addClass = self.__getIconAnchorAndClass(contentIconType=contentObjName, cssClass='pull-right')
        self.__html.beginContainer()
        self.__html.beginPanel(panelTitle + iconText, style='panel-default ' + addClass, format='html')
        self.__html.beginInLineList()
        self.__html.addInLineList(valueList, format=format)
        self.__html.endInLineList()
        self.__html.endPanel()
        self.__html.endContainer()

    def __addAccordionPanelWithTable(self, rowList, columnNameList, panelTitle, contentObjName='default', columnNameFormat='ascii', dataFormat='ascii',
                                     newLines='verbatim', idSuffix='a', openFlag=True, toggleText='View/Hide Table'):
        """
        """
    ...
    def __renderItemPage(self, itemName):
        ...
        iconText, addClass = self.__getIconAnchorAndClass(contentIconType='info', cssClass='pull-right')
        self.__html.beginContainer(type='row')
        self.__html.beginPanel('Item Description ' + iconText, style='panel-default ' + addClass, format='html')
        #ht=self.__mU.getFormatted(description,wrapper='pre',  format='ascii')
        ht = self.__mU.getFormatted(description, wrapper='verbatim', format='ascii', markupMath=True)
        self.__html.addContent([ht])
        self.__html.endPanel()
        self.__html.endContainer()
        ...
        htmlList = self.__renderExamples(altExampleList, 'Additional Item Example for Depositors',
                                         'Additional Item Examples for Depositors', idSuffix='b')
        ...
        self.__html.beginPanel('Data Type ' + iconText, style='panel-default ' + addClass, format='html')
        self.__html.beginContainer()
        self.__html.beginDescriptionList()
        ...
        if typeCode is not None and len(typeCode) > 0:
            ...
            logger.warn("+ERROR missing type for item %s\n" % itemName)
        ...
        if enumListWithDetail is not None and len(enumListWithDetail) > 0:
            ...
            self.__addAccordionPanelWithTable(rowList=enumListWithDetail, columnNameList=columnNameList, panelTitle='Controlled Vocabulary', contentObjName='default',
                                              columnNameFormat='html', dataFormat='ascii', newLines='verbatim', idSuffix='enum', openFlag=True, toggleText='View/Hide Table')
        ...
        if enumListAltWithDetail is not None and len(enumListAltWithDetail) > 0:
            columnNameList = ['Allowed&nbsp;Value', 'Details']
            self.__addAccordionPanelWithTable(rowList=enumListAltWithDetail, columnNameList=columnNameList,
                                              panelTitle='Controlled Vocabulary at Deposition', contentObjName='default',
                                              columnNameFormat='html', dataFormat='ascii', newLines='verbatim',
                                              idSuffix='enumalt', openFlag=True, toggleText='View/Hide Table')

        #
        # Boundary values -
        #  (min, max)
        skipEquivalentBounds = True
        bList = self.__dApi.getBoundaryList(categoryName, attributeName)
        boundaryList = []
        for b in bList:
            min = b[0]
            max = b[1]
        ...
        if boundaryList is not None and len(boundaryList) > 0:
            ...
            self.__addPanelWithTable(rowList=boundaryList, columnNameList=columnNameList, panelTitle='Allowed Boundary Conditions', contentObjName='default',
                                     columnNameFormat='html', dataFormat='html', newLines='verbatim')
        #
        bList = self.__dApi.getBoundaryListAlt(categoryName, attributeName, fallBack=False)
        boundaryListAlt = []
        for b in bList:
            min = b[0]
            max = b[1]
            ...
            if max == '.':
                max = '<h3>+&infin;</h3>'
            boundaryListAlt.append((min, max))

        if boundaryListAlt is not None and len(boundaryListAlt) > 0:
            columnNameList = ['Minimum&nbsp;Value', 'Maximum&nbsp;Value']
            self.__addPanelWithTable(rowList=boundaryListAlt, columnNameList=columnNameList, panelTitle='Advisory Boundary Conditions', contentObjName='default',
                                     columnNameFormat='html', dataFormat='html', newLines='verbatim')
        #
        # --
        # Parent-child -
        ...
        if parentList is not None and len(parentList) > 0:
            ...
            if uP != parentList[0]:
                ...
                logger.debug("+INFO - Ultimate parent of %s is %s\n" % (parentList[0], uP))
            if len(parentList) > 1:
                logger.debug("+WARN - Multiple parents for %s  %s %r\n" % (categoryName, attributeName, parentList))
        ...
        if relatedList is not None and len(relatedList) > 0:
            ...
            for itemName, relationType in relatedList:
                ...
                linkAnchor = self.__mU.getAnchor(contentUrl=linkUrl, contentLabel=itemName, cssClassString=None, format="ascii")
                rList.append((linkAnchor, relationType))

            columnNameList = ['Related&nbsp;Item&nbsp;Name', 'Relation&nbsp;Type']
            self.__addPanelWithTable(rowList=rList, columnNameList=columnNameList, panelTitle='Related Items', contentObjName='related-item',
                                     columnNameFormat='html', dataFormat='html', newLines='verbatim')
        #
        ...
        if subCategoryList is not None and len(subCategoryList) > 0:
            ...
            self.__addPanelWithTable(rowList=dList, columnNameList=columnNameList, panelTitle='Subcategories', contentObjName='info',
                                     columnNameFormat='html', dataFormat='ascii', newLines='verbatim')
        #
        # --  aliases
        #
        ...
        if aliasTupleList is not None and len(aliasTupleList) > 0:
            ...
            self.__addPanelWithTable(rowList=aliasTupleList, columnNameList=columnNameList, panelTitle='Aliases', contentObjName='info',
                                     columnNameFormat='html', dataFormat='ascii', newLines='verbatim')
