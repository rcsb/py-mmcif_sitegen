...
class HtmlPathInfo(object):
    ...
    def getContentTypeDisplayName(self, contentType):
        try:
            ...
            logger.error("HtmlGenerator.getContentTypeDisplayName() failed for %s\n" % contentType)
    ...
    def __getDictName(self, dictFilePath):
        ...
        try:
            ...
            logger.error("HtmlGenerator.__getDictName() failed for %s\n" % dictFilePath)
            logger.exception("Failing with %s" % str(e))
        return None
    ...
    def __setTopPath(self, htmlDocsPath, htmlTopDir):
        ...
        try:
            ...
            if not os.access(htmlDocsPath, os.W_OK):
                return None
            return os.path.join(htmlDocsPath, htmlTopDir)

        except Exception as e:
            logger.error("HtmlGenerator.setTopPath() setting to content path failed for %s %s \n" % (htmlDocsPath, htmlTopDir))
            logger.exception("Failing with %s" % str(e))
        return None
