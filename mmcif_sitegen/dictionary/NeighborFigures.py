...
class NeighborFigures(object):
    ...
    def setItemCounts(self, itemNameD, deliveryType='archive'):
        ...
        logger.debug("+NeighborFigures.setItemCounts() items      in archive count %d\n" % len(self.__itemCounts[deliveryType]))
        logger.debug("+NeighborFigures.setItemCounts() categories in archive count %d\n" % len(self.__categoryCounts[deliveryType]))
    ...
    def __getOrderedItemNameList(self, categoryName, fkList=[], filterDelivery=False, deliveryType='archive'):
        ...
        for keyItemName in keyItemNameList:
            ...
            if keyItemName in tList:
                ...
                logger.warn("+NeighborFigures.__getOrderedItemNameList() Category %s is missing key definition for %s\n" % (categoryName, keyItemName))
    ...
    def __renderCategory(self, categoryName, fkList=[], highLight='adjacent', maxItems=10, filterDelivery=False, deliveryType='archive'):
    ...
    def __getRelatedList(self, categoryName, adjacentD):
        ...
        logger.debug("+NeighborFigures.__getRelatedList() %s items with parent/child relationships %s\n" % (categoryName, len(relatedList)))
        #
        return relatedList

    def __generateDotInstructions(self, categoryName, graphTitle=None, graphSubTitle=None, titleFormat='text', maxItems=20,
                                  filterDelivery=False, deliveryType='archive',
                                  neighborCategoryList=None, maxCategories=None):
        ...
        #
        for k, v in aR.items():
            logger.debug("+NeighborFigures.__generateDotInstructions() %s relatives %s\n" % (k, v))
        #
        adjacentCategories = []
        ...
        logger.debug("+NeighborFigures.__generateDotInstructions() adjacent categories %s\n" % adjacentCategories)
        #
        oL = []
        ...
        if graphTitle is not None:
            if titleFormat == 'text':
                ...
                oL.append('graph [labelloc=b, labeljust=left, labelfontname=Helvetica, labelfontsize=18, label="%s"];' %
                          (self.__titleFontFace, self.__titleFontSize, graphTitle))
            else:
        ...
        for catName in adjacentCategories:
            ...
            oL.extend(self.__renderCategory(catName, fkList=relatedList, highLight=highLight, maxItems=maxItems,
    ...
    def makeNeighborFigure(self, categoryName, graphTitle=None, graphSubTitle=None, titleFormat='text', format='svg',
                           size=None, maxItems=20, filterDelivery=False, deliveryType='archive',
                           neighborCategoryList=None, maxCategories=None, cleanup=False, imageFilePath=None):
        ...
        """
        dotList, numCategoriesRendered = self.__generateDotInstructions(categoryName,
                                                                        graphTitle=graphTitle, graphSubTitle=graphSubTitle, titleFormat=titleFormat,
                                                                        maxItems=maxItems, filterDelivery=filterDelivery, deliveryType=deliveryType,
                                                                        neighborCategoryList=neighborCategoryList, maxCategories=maxCategories)
        ...
        #
        if size is not None:
            cmd = self.__pathDot + ' -T%s  -Gsize="%s" %s > %s' % (format, size, dotfn, svgfn)
        else:
            cmd = self.__pathDot + ' -T%s %s > %s' % (format, dotfn, svgfn)
        ok = os.system(cmd)
        #
        # Remove any failed image files --
        if ok != 0:
            logger.debug("+NeighborFigures.makeNeighborFigure() status %s for %s\n" % (ok, svgfn))
            cmd = "rm -f  %s" % svgfn
            os.system(cmd)
        #
        if cleanup:
