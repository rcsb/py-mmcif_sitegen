...
##
"""
Tests cases for the dictionary rendering workflow class.
...
class HtmlGeneratorWfTests(unittest.TestCase):
    def setUp(self):
        self.__verbose = True
        #
    ...
    def testWorkflow(self):
        ...
        try:
            hgWf = HtmlGeneratorWf(websiteGenPath=self.__websiteGenPath, websiteFileAssetsPath=self.__websiteFileAssetsPath, testMode=self.__testModeFlag)
            ok = hgWf.run()
            self.assertTrue(ok)
...
def suiteWorkflowTests():
    ...
    suiteSelect.addTest(HtmlGeneratorWfTests("testWorkflow"))
    return suiteSelect
